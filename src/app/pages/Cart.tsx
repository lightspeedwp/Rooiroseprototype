...
export const CartPage = () => {
      ...
      <div className="bg-white dark:bg-background min-h-screen pb-20">
        <PageContainer breadcrumbs={[{ label: 'Mandjie' }]}>
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-24 h-24 bg-gray-50 dark:bg-card rounded-full flex items-center justify-center mb-6">
  ...
  return (
    <div className="bg-white dark:bg-background min-h-screen pb-20">
      <PageContainer breadcrumbs={[{ label: 'Mandjie' }]}>
        <h1
          className="text-4xl md:text-5xl font-normal text-brand-navy dark:text-foreground my-8 md:my-12 font-heading"
          style={h1Style}
        >Mandjie</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          <div className="flex-1">
            ...
            <div className="hidden md:flex justify-between border-b border-gray-200 dark:border-border pb-4 mb-6">
              <span className="text-xs font-bold text-gray-500 dark:text-gray-400 tracking-wider">PRODUK</span>
              <span className="text-xs font-bold text-gray-500 dark:text-gray-400 tracking-wider">TOTAAL</span>
            </div>

            <div className="space-y-8 md:space-y-10">
              {items.map((item) => (
                <div key={item.productId} className="flex flex-col md:flex-row gap-6 border-b border-gray-100 dark:border-border pb-8 md:border-none md:pb-0">
                  {/* Image */}
                  <div className="w-full md:w-32 h-32 bg-gray-50 dark:bg-card rounded-lg overflow-hidden flex-shrink-0">
                    {item.image ? (
                  ...
                  <div className="flex-1 flex flex-col md:flex-row justify-between">
                    <div className="flex-1 pr-4">
                      <div className="flex justify-between md:block mb-4 md:mb-0">
                        <h3 className="text-lg font-normal text-brand-navy dark:text-foreground mb-1 font-heading">
                          {item.title}
                        </h3>
                        {/* Mobile Price */}
                        <div className="md:hidden font-medium text-brand-navy dark:text-foreground">
                          R {(item.price * item.quantity).toFixed(2)}
                        </div>
                      </div>
                      
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-medium text-brand-navy dark:text-foreground">R {item.price.toFixed(2)}</span>
                      </div>
                      
                      <div className="flex items-center gap-2 mb-6">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${item.type === 'subscription' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300' : 'bg-gray-100 text-gray-600 dark:bg-white/10 dark:text-white/50'}`}>
                          {item.type === 'subscription' ? 'Intekening' : 'Eenmalig'}
                        </span>
                        {item.variantLabel && (
                          <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300">
                            Streek: {item.variantLabel}
                          </span>
                        )}
                        <span className="text-gray-500 dark:text-gray-400 text-sm">
                          {item.type === 'subscription' ? 'Digitale Intekening' : 'Enkel Uitgawe'}
                        </span>
                      </div>

                      {/* Quantity & Remove Row */}
                      <div className="flex items-center gap-6">
                        <div className="flex items-center border border-gray-200 dark:border-input rounded px-2 h-10 w-32">
                          <button 
                          ...
                          <input 
                            ...
                            value={item.quantity}
                            readOnly
                            className="flex-1 w-full text-center border-none focus:ring-0 p-0 text-sm font-medium text-brand-navy dark:text-foreground bg-transparent"
                          />
                          <button 
                        ...
                        <button 
                          ...
                          className="text-sm text-gray-500 dark:text-gray-400 underline hover:text-primary dark:hover:text-primary transition-colors"
                        >
                          Verwyder item
                        </button>
                      </div>
                    ...
                    {/* Desktop Total */}
                    <div className="hidden md:block text-right font-medium text-lg text-brand-navy dark:text-foreground">
                      R {(item.price * item.quantity).toFixed(2)}
                    </div>
                  </div>
                </div>
          ...
          <div className="w-full lg:w-[400px] flex-shrink-0">
            <h2 className="text-xs font-normal text-gray-500 dark:text-gray-400 tracking-wider uppercase mb-6 font-heading" style={{ fontVariationSettings: "var(--fvs-h2)", lineHeight: 'var(--lh-h2)', letterSpacing: 'var(--ls-h2)' }}>Mandjie Totaal</h2>
            
            <div className="bg-gray-50 dark:bg-card rounded-lg p-6 md:p-8">
              {/* Coupon */}
              <div className="border-b border-gray-200 dark:border-border pb-6 mb-6">
