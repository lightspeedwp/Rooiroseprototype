...
/* ── rooi rose Magazine My Account Page ──────────────────────────────
 * Editorial design: WooCommerce account dashboard
    ...
    return (
      <div className="bg-white dark:bg-background min-h-screen">
        <PageContainer breadcrumbs={[{ label: 'My rekening' }]}>
          <div className="py-12">
            <h1
              className="text-4xl font-normal text-center text-brand-navy dark:text-foreground mb-12 has-brand-serif-font-family"
              style={{ fontVariationSettings: "var(--fvs-h1)", lineHeight: 'var(--lh-h1)', letterSpacing: 'var(--ls-h1)' }}
            >My rekening</h1>
            
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              {/* Login Form */}
              <div className="bg-white dark:bg-card p-8 rounded-xl shadow-md dark:shadow-[var(--shadow-dark-300)] border border-gray-200 dark:border-border">
                <h2 className="text-2xl font-normal mb-6 text-brand-navy dark:text-foreground has-brand-serif-font-family border-b-2 border-brand-red pb-3 inline-block" style={{ fontVariationSettings: "var(--fvs-h2)", lineHeight: 'var(--lh-h2)', letterSpacing: 'var(--ls-h2)', fontSize: 'var(--text-h2)' }}>
                  Teken in
                </h2>
                <form className="space-y-5 mt-6" onSubmit={handleLogin}>
                  <div className="space-y-2">
                    <Label htmlFor="email" className="text-sm font-bold text-gray-700 dark:text-gray-300">E-posadres of gebruikersnaam</Label>
                    <Input id="email" type="email" placeholder="naam@voorbeeld.co.za" required className="h-12" />
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <Label htmlFor="password" className="text-sm font-bold text-gray-700 dark:text-gray-300">Wagwoord</Label>
                      <button type="button" className="text-xs text-brand-red hover:underline font-medium" onClick={() => toast.info("Wagwoord herstel e-pos gestuur.")}>
                        Wagwoord vergeet?
                      </button>
                    </div>
                    <Input id="password" type="password" required className="h-12" />
                  </div>
                  <Button type="submit" className="w-full bg-brand-red hover:bg-brand-red/90 text-white font-bold h-12 mt-6" disabled={loading}>
              ...
              {/* Register Info */}
              <div className="bg-gray-50 dark:bg-card p-8 rounded-xl border border-gray-200 dark:border-border shadow-md dark:shadow-[var(--shadow-dark-300)] flex flex-col justify-center">
                <h2 className="text-2xl font-normal mb-4 text-brand-navy dark:text-foreground has-brand-serif-font-family border-b-2 border-brand-red pb-3 inline-block" style={{ fontVariationSettings: "var(--fvs-h2)", lineHeight: 'var(--lh-h2)', letterSpacing: 'var(--ls-h2)', fontSize: 'var(--text-h2)' }}>
                  Registreer
                </h2>
                <p className="text-gray-600 dark:text-gray-400 mb-6 leading-relaxed mt-6">
                ...
                <div className="space-y-4">
                   ...
                   <Button asChild variant="outline" className="w-full h-12 border-2 border-brand-red text-brand-red hover:bg-brand-red hover:text-white transition-colors font-bold">
        ...
        <div className="py-12">
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <nav className="w-full md:w-72 flex-shrink-0">
               <div className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-border overflow-hidden shadow-md dark:shadow-[var(--shadow-dark-300)]">
                 <div className="p-5 bg-brand-navy dark:bg-brand-navy-dark border-b border-brand-navy-dark flex items-center gap-3">
                   <div className="h-12 w-12 rounded-full bg-brand-red text-white flex items-center justify-center font-bold text-lg">
                     {MOCK_USER.name.charAt(0)}
                   </div>
                   <div className="overflow-hidden">
                     <p className="text-sm font-bold truncate text-white">{MOCK_USER.name}</p>
                     <p className="text-xs text-gray-300 truncate">{MOCK_USER.email}</p>
                   </div>
                 </div>
                 ...
                 <div className="flex flex-col">
                   ...
                   <div className="border-t border-gray-100 dark:border-border mt-2">
                     <button 
                       ...
                       className="w-full text-left px-5 py-3 text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-600 dark:hover:text-red-400 transition-colors flex items-center gap-3"
                     >
                       <LogOut size={18} />
                       Teken uit
                     </button>
            ...
            <div className="flex-grow">
              <div className="bg-white dark:bg-card p-8 rounded-xl border border-gray-200 dark:border-border shadow-md dark:shadow-[var(--shadow-dark-300)] min-h-[500px]">
                {activeTab === 'dashboard' && (
                  <div className="space-y-6 fade-in">
                    <p className="text-gray-700 dark:text-gray-300 text-lg leading-relaxed">
                      Hallo <strong className="text-brand-navy dark:text-foreground">{MOCK_USER.name}</strong> (nie <strong>{MOCK_USER.name}</strong> nie? <button onClick={handleLogout} className="text-brand-red hover:underline font-medium">Teken uit</button>)
                    </p>
                    <p className="text-gray-600 dark:text-gray-400 leading-relaxed">
                      Vanuit jou rekeningpaneelbord kan jy jou <button onClick={() => setActiveTab('orders')} className="text-brand-red hover:underline font-medium">onlangse bestellings</button> sien, jou <button onClick={() => setActiveTab('addresses')} className="text-brand-red hover:underline font-medium">versendings- en faktuuradresse</button> bestuur, en jou <button onClick={() => setActiveTab('account')} className="text-brand-red hover:underline font-medium">wagwoord en rekeningbesonderhede</button> wysig.
                    </p>
